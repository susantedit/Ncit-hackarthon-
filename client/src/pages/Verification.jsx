...
import {
  ...
  Search, ChevronRight, ArrowUpRight, TriangleAlert, Layers3, ScanLine, Volume2, Brain
} from 'lucide-react'
...
const verdictMeta = {
  'True':             { label: 'True',             color: '#10b981', bg: 'rgba(16,185,129,0.12)',  border: 'rgba(16,185,129,0.28)',  Icon: CheckCircle2 },
  'Mostly True':      { label: 'Mostly True',      color: '#22c55e', bg: 'rgba(34,197,94,0.12)',   border: 'rgba(34,197,94,0.28)',   Icon: CheckCircle2 },
  'Partially True':   { label: 'Partially True',   color: '#38bdf8', bg: 'rgba(56,189,248,0.12)',  border: 'rgba(56,189,248,0.28)',  Icon: Layers3 },
  'Unable to Verify': { label: 'Unable to Verify', color: '#f59e0b', bg: 'rgba(245,158,11,0.12)',  border: 'rgba(245,158,11,0.28)',  Icon: MinusCircle },
  'Misleading':       { label: 'Misleading',       color: '#fb7185', bg: 'rgba(251,113,133,0.12)', border: 'rgba(251,113,133,0.30)', Icon: AlertTriangle },
...
function formatScore(value) {
  if (Number.isNaN(value) || value == null) return '0/100'
...
export default function Verification() {
  ...
  const rawVerdict = result?.verdict || LEGACY_VERDICT_MAP[result?.summary?.overallStatus] || 'Unable to Verify'
  const currentVerdict = verdictMeta[rawVerdict] ? rawVerdict : (LEGACY_VERDICT_MAP[rawVerdict] || 'Unable to Verify')
  const verdictInfo = verdictMeta[currentVerdict] || verdictMeta['Unable to Verify']
  const liveClaimCount = result?.claims?.length || 0
  const confidence = result?.confidenceScore ?? result?.summary?.confidence ?? 0
  ...
  const speakSummary = async () => {
    ...
    try {
      if (!consentGiven) {
        toast.error('Please consent to sending content to external verification and TTS services')
        setVoiceLoading(false)
        return
      }

      const payload = { inputText, sourceUrl, imageText, inputType: mode, ttsVoice: 'alloy', consent: true }
      const res = await api.verifyClaimsWithTTS(payload)
      const voiceUrl = res.tts?.audio || res.tts?.audioUrl || ''
      if (voiceUrl) {
        setVoiceAudioUrl(voiceUrl)
        setLiveStatus('Voice summary ready')
      } else if (res.tts && res.tts.error) {
        toast.error(res.tts.error)
      }
      if (res.verification) setResult(res.verification)
    } catch (err) {
    ...
    <div className="page-wrapper" style={{ background: '#07101f', minHeight: '100vh' }}>
      ...
      <div className="page-content-wide" style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, color: 'var(--text2)', marginTop: 2 }}>
                  Sathi AI fact checker retrieves evidence first, then synthesizes a grounded verdict — never guesses.
                </p>
              </div>
            </div>
        ...
        <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
          <input id="consent-tts" type="checkbox" checked={consentGiven} onChange={e => setConsentGiven(e.target.checked)} />
          <label htmlFor="consent-tts" style={{ fontSize: 12, color: 'var(--text3)' }}>I consent to sending the provided content to external search and TTS services for verification and audio synthesis.</label>
        </div>
                  ...
                  {result && (
                    <div style={{ marginTop: 8 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--text3)', marginBottom: 4 }}>
                        ...
                        <span style={{ color: verdictInfo.color, fontWeight: 800 }}>{confidence}%</span>
                      </div>
                      <div style={{ height: 6, borderRadius: 3, background: 'rgba(255,255,255,0.08)', overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${confidence}%`, borderRadius: 3, background: verdictInfo.color, transition: 'width 0.6s ease' }} />
                      </div>
                    </div>
                  )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {result?.aiReasoning && (
              <FloatingPanel glowColor="#a855f7" style={{ padding: 20 }}>
                <SectionLabel icon={Brain} title="AI Reasoning" subtitle="Evidence-based analysis from Sathi AI" />
                <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.8, fontStyle: 'italic' }}>
                  {result.aiReasoning}
                </div>
              </FloatingPanel>
            )}
            <FloatingPanel glowColor={verdictInfo.color} style={{ padding: 20 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {groupedClaims.length ? groupedClaims.map(claim => {
                  const claimVerdictKey = verdictMeta[claim.verdict] ? claim.verdict : (LEGACY_VERDICT_MAP[claim.verdict] || 'Unable to Verify')
                  const claimVerdictInfo = verdictMeta[claimVerdictKey] || verdictMeta['Unable to Verify']
                    ...
                    <div key={`${claim.index}-${claim.claim}`} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
                        <div style={{ minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
                            <div style={{ padding: '4px 8px', borderRadius: 999, background: claimVerdictInfo.bg, border: `1px solid ${claimVerdictInfo.border}`, color: claimVerdictInfo.color, fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                              <VerdictIcon size={11} /> {claimVerdictKey}
                            </div>
                        ...
                        <div style={{ flexShrink: 0, textAlign: 'right' }}>
                          <div style={{ fontSize: 11, color: 'var(--text3)' }}>Confidence score</div>
                          <div style={{ fontSize: 14, fontWeight: 900, color: claimVerdictInfo.color }}>{formatScore(claim.confidence)}</div>
                        </div>
                      </div>
                      ...
                      <div style={{ padding: 12, borderRadius: 14, background: 'rgba(11,15,26,0.70)', border: '1px solid rgba(255,255,255,0.06)', marginBottom: 12 }}>
                        <div style={{ fontSize: 11, fontWeight: 800, color: '#22d3ee', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>Evidence takeaway</div>
