...
const MURF_API_URL = process.env.MURF_API_URL || ''

async function synthesizeMurfText(text, voice = 'alloy', format = 'mp3') {
  if (!MURF_API_KEY || !MURF_API_URL) {
    throw new Error('MURF_API_KEY or MURF_API_URL not configured')
  }

  // This is a generic POST to a Murf-like TTS endpoint. Replace with
  // the real Murf API contract and fields when deploying.
  const payload = {
    input: text,
    voice: voice,
    format: format,
  ...
  const res = await axios.post(`${MURF_API_URL}/v1/tts`, payload, {
    headers: {
  ...
  // Return base64-encoded audio so callers can play immediately in-browser
  const audioBase64 = Buffer.from(res.data, 'binary').toString('base64')
  const mime = format === 'wav' ? 'audio/wav' : 'audio/mpeg'
  return `data:${mime};base64,${audioBase64}`
}

export { synthesizeMurfText }
